...
# ===============================
def start_game():
    global snake, dx, dy, score, paused, pillars
...
def draw_menu():
    WIN.fill(BG_COLOR)
    draw_text(WIN, "Snake Obstacle", (WIDTH//2, HEIGHT//4), FONT)
...
def draw_game(delta_time):
    ...
    for i, seg in enumerate(snake):
        if i==0:
            ...
            elif seg[1]>prev[1]: t=snake_tail_img
            WIN.blit(t,seg)
        else:
            WIN.blit(snake_body_img,seg)

    # Vykreslení jablka
    WIN.blit(apple_img,food[0] if food else [0,0])
...
while True:
    ...
    elif screen_state=="game": draw_game(delta_time)
